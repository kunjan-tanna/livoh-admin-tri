...
class PrivacyPolicy extends Component {
    ...
    async componentDidMount() {
        ...
        window.CKEDITOR.replace("2", configuration);
        window.CKEDITOR.inline("2");
        // window.CKEDITOR.plugins.addExternal( 'imageresize', '../imageresize-master/plugin', 'plugin.js' );
        window.CKEDITOR.instances["2"].on("change", function () {
            let data = window.CKEDITOR.instances[2].getData();
            this.setTxt(data);
        }.bind(this));

        window.CKEDITOR.instances["2"].on("fileUploadRequest", function () {
            store.dispatch({
                type: 'START_LOADER'
            })
        }.bind(this));

        window.CKEDITOR.instances["2"].on("fileUploadResponse", function () {
            store.dispatch({
                type: 'STOP_LOADER'
            })
        }.bind(this));
    ...
    getData = async () => {
        ...
        // let name = String(this.props.path).substr(index + 2, this.props.path.length);
        // let response;
        console.log('name->', name);
        Emit('adminConfig');
        GetResponse(res => {
            console.log('\n\n RES77777->', res);
            this.setState({ text: res.data.privacy_policy_text , id: res.data.admin_config_id }, () => {
            });
        })
    }
    onChangeText = (evt) => {
    ...
    onSubmitHandler = async () => {
        let reqData = {
            id: this.state.id,
            privacy_policy_text: this.state.text
        }
                            ...
                            <CardBody>
                                <Row>
                                    <Col>
                                        <div>
                                            <FormGroup>
                                                <Input type="textarea" name={"2"} id={"2"} value={this.state.text} onChange={() => this.onChangeText()} />
                                            </FormGroup>
                                        </div>
