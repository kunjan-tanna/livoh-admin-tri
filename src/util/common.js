...
export const apiCall = async (method, url, reqData, params, header) => {
  return new Promise((resolve, reject) => {
    ...
    if (header) {
      ...
      headers = {
        "language": "en",
        "auth_token": "@#Slsjpoq$S1o08#MnbAiB%UVUV&Y*5EU@exS1o!08L9TSlsjpo#TEKOTO",
      };
    }
    if (localStorage.getItem('DUDU_AUTH_TOKEN') !== null) {
      headers.auth_token = localStorage.getItem('DUDU_AUTH_TOKEN');
    }
    axios({
      method: method,
      url: url,
      ...
      .then((response) => {
        ...
        console.log('\n\n\n 2222222222222222222222222', response);
